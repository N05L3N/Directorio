...
$tijuana2 = '200.76.235.250';
$torreon = '187.216.28.34';
$veracruz = '187.217.113.50';

	if ($ip == $aguascalientes OR $ip == $chihuahua OR $ip == $culiacan OR $ip == $durango OR $ip == $guadalajara OR $ip == $hermosillo OR $ip == $juarez OR $ip == $leon OR $ip == $mexicali OR $ip == $monterrey OR $ip == $obregón OR $ip == $puebla OR $ip == $querétaro OR $ip == $saltillo OR $ip == $sanluispotosi OR $ip == $tijuana OR $ip == $tijuana2 OR $ip == $torreon OR $ip == $veracruz) {
#	if ($ip != $chihuahua) {
		
		if ($_SESSION['intranet_tkn'] == 1) {
			$error = '';
			$id_url = 'consultas';
			include('inc/head.personal.php');
			# Vistas
